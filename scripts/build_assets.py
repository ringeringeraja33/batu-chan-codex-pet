#!/usr/bin/env python3
"""Build the Batu Codex v2 atlas and preview assets from chroma-key sheets."""
...
SOURCE = ROOT / "source" / "batu-pose-sheet-downturned-fur-brim-v19-chroma.png"
...
def remove_magenta_chroma(image: Image.Image) -> Image.Image:
    """Turn the generated magenta backdrop into clean straight-alpha RGBA."""
    rgb = image.convert("RGB")
    keyed = Image.new("RGBA", rgb.size, (0, 0, 0, 0))
    output: list[tuple[int, int, int, int]] = []
    for red, green, blue in rgb.get_flattened_data():
        key_distance_sq = (red - 238) ** 2 + (green - 6) ** 2 + (blue - 233) ** 2
        key_strength = min(red, blue) - green
        if key_distance_sq <= 120**2:
            alpha = 0
        elif red > 95 and blue > 95 and key_strength > 55:
            alpha = round(255 * max(0.0, min(1.0, (145 - key_strength) / 90)))
        else:
            alpha = 255

        # Very low-opacity key remnants become a purple halo after WebP
        # filtering, so drop them instead of preserving meaningless coverage.
        if alpha <= 96:
        ...
        if alpha < 255:
            ...
            red = max(green, red - round(magenta_excess * (1 - alpha / 255)))
            blue = max(green, blue - round(magenta_excess * (1 - alpha / 255)))
        output.append((red, green, blue, alpha))

    keyed.putdata(output)
    return remove_transparent_rgb(keyed)
...
def extract_animation_rows(sheet: Image.Image) -> tuple[list[Image.Image], list[Image.Image]]:
    ...
    for row in range(2):
        ...
        for cell, bound in zip(cells, visible_bounds):
            ...
            weights = list(lower.get_flattened_data())
            total = sum(weights)
            if not total:
...
def main() -> None:
    source = remove_magenta_chroma(Image.open(SOURCE))
